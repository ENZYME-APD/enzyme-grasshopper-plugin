    ...
    for line in inputs_block.split(';'):
        ...
        if "AddNumberParameter" in line:
            if "GH_ParamAccess.item" in line:
                ...
                val = num_match.group(1) if num_match else "1.5"
...
def process_file(filepath):
    ...
    # Input y spacing = 40 (matches standard param height)
    valid_inputs = [inp for inp in inputs if inp[0] != "unknown"]
    total_in_h = len(valid_inputs) * 40
    in_y_start = -(total_in_h - 40) // 2 if valid_inputs else 0
    
    y_offset = in_y_start
    # IX offsets: The larger the number, the further left they are. 
    # To right-justify them at ~150px left of the component center:
    # slider ~180px wide -> 180 + 150 = 330
    # toggle ~60px wide -> 60 + 150 = 210
    # button ~60px wide -> 60 + 150 = 210
    # panel ~100px wide -> 100 + 150 = 250
    # valuelist ~150px wide -> 150 + 150 = 300
    # color ~60px wide -> 60 + 150 = 210
    for i, inp in enumerate(inputs):
        ...
        if t == "slider_num":
            ...
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireSlider(this, document, {i}, 0.0, {max_val}, {inp[2]}, 330, {y_offset});')
        elif t == "slider_int":
            max_val = get_slider_max(inp[2])
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireSlider(this, document, {i}, 0.0, {max_val}, {inp[2]}, 330, {y_offset});')
        elif t == "toggle":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireToggle(this, document, {i}, {inp[2]}, 210, {y_offset});')
        elif t == "button":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireButton(this, document, {i}, 210, {y_offset});')
        elif t == "valuelist":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireValueList(this, document, {i}, new string[]{{{inp[2]}}}, new string[]{{{inp[3]}}}, 300, {y_offset});')
        elif t == "panel":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WirePanel(this, document, {i}, "{inp[2]}", 250, {y_offset}, 100, 25);')
        elif t == "color":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireColorSwatch(this, document, {i}, System.Drawing.Color.{inp[2]}, 210, {y_offset});')
    ...
    for out in valid_outputs:
        out_total_h += 75 if out[0] == "preview" else 45
        
    out_y_offset = -(out_total_h - 45) // 2 if valid_outputs else 0
    
    # OX offsets: The larger the number, the further right they are.
    # Set to 220 to avoid wide components.
    base_ox = 220
    for i, out in enumerate(outputs):
        ...
        if t == "preview":
            ...
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireCustomPreview(this, document, {i}, {color}, {base_ox}, {out_y_offset});')
            out_y_offset += 75
        elif t == "curve":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "curve", {base_ox}, {out_y_offset});')
            out_y_offset += 45
        elif t == "point":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "point", {base_ox}, {out_y_offset});')
            out_y_offset += 45
        elif t == "line":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "line", {base_ox}, {out_y_offset});')
            out_y_offset += 45
        elif t == "panel":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputPanel(this, document, {i}, {base_ox}, {out_y_offset - 11}, 180, 22);')
            out_y_offset += 45
...
print(f"Processed {processed} files for auto-wiring improvements v6.")
