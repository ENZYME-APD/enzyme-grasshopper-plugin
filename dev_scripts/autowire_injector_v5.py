...
# We'll process all files that have a RegisterInputParams block
...
def parse_inputs(content):
    ...
    for line in inputs_block.split(';'):
        ...
        is_list_or_tree = "GH_ParamAccess.list" in line or "GH_ParamAccess.tree" in line
        ...
        if num_match:
            inputs.append(("slider_num", name, num_match.group(1)))
            continue
        ...
        if int_match and not int_options:
            inputs.append(("slider_int", name, int_match.group(1)))
            continue
...
def process_file(filepath):
    ...
    # Skip files that shouldn't have auto-wiring (e.g. utilities with custom wiring like PluginInfo)
    ...
    # Remove existing AddedToDocument block if it exists
    ...
    # Count how many valid auto-wired inputs we have
    ...
    # Custom previews need more vertical space to avoid swatch overlaps
    out_y_offset = 0
    out_total_h = 0
    valid_outputs = [out for out in outputs if out[0] != "unknown"]
    for out in valid_outputs:
        out_total_h += 50 if out[0] == "preview" else 30
    ...
    for i, out in enumerate(outputs):
        ...
        if t == "preview":
            ...
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireCustomPreview(this, document, {i}, {color}, 200, {out_y_offset});')
            out_y_offset += 50
        elif t == "curve":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "curve", 100, {out_y_offset});')
            out_y_offset += 30
        elif t == "point":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "point", 100, {out_y_offset});')
            out_y_offset += 30
        elif t == "line":
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireOutputParam(this, document, {i}, "line", 100, {out_y_offset});')
            out_y_offset += 30
        elif t == "panel":
