"""
OPP MASTERPLAN ENGINE (JSON-BUS EDITION) - V2.1
================================================================================
...
* UPDATED: Facade JSON now uses native `BoundsExt` and `BoundsClosed` naming.
================================================================================
"""
import json
...
ghenv.Component.Name = "JSON MP Engine V2"
...
def get_exterior_segments(crv, master_crvs, tolerance=0.05):
    if not crv or not master_crvs: return []
    segments = crv.DuplicateSegments()
    if not segments: segments = [crv]
    ext_segments = []
    for seg in segments:
        mid_pt = seg.PointAtNormalizedLength(0.5)
        is_exterior = False
        for m_crv in master_crvs:
            rc, t = m_crv.ClosestPoint(mid_pt)
            if rc and m_crv.PointAt(t).DistanceTo(mid_pt) <= tolerance:
                is_exterior = True
                break
        if is_exterior:
            ext_segments.append(seg)
    if ext_segments:
        joined = rg.Curve.JoinCurves(ext_segments, 0.01)
        return list(joined) if joined else ext_segments
    return []
    ...
    def generate_topology(self):
        for tid, t_blocks in self.tower_groups.items():
            ...
            for rel_z in sorted(z_dict.keys()):
                ...
                if rel_z not in self.master_slabs:
                    self.master_slabs[rel_z] = []
                self.master_slabs[rel_z].extend([c.Duplicate() for c in final_slab_crvs])
                ...
                intersect_crvs = safe_boolean_op(final_slab_crvs, unioned_bounds_above, "int", 0.001)
                
                slab_area = get_brep_area(final_slab_crvs)
                ...
                if is_roof:
                    ...
                    self.roof_json_data.append({
                        ...
                        "slab_bounds": [serialize_exact_curve(c) for c in final_slab_crvs],
...
masses_dict, slabs_dict, roofs_dict, facade_dict, railings_dict = {}, {}, {}, {}, {}
dash_total_area, dash_programs, dash_buildings = 0.0, {}, {}

if not json_in:
    ghenv.Component.Message = "MP ENGINE V2\nTime: 0.0 ms\n---\nAwaiting Payload"
else:
    ...
    for bldg_data in data.get("buildings", []):
        ...
        for block in bldg.blocks:
            ...
            if block.program not in facade_dict[b_name]: facade_dict[b_name][block.program] = []
                
            if block.base_curve:
                for i in range(block.floors):
                    rel_z = round(block.base_z + (i * block.floor_height), 3)
                    floor_true_z = round(bldg.elevation + rel_z, 3)
                    
                    master_slabs = bldg.master_slabs.get(rel_z, [])
                    ext_crvs = get_exterior_segments(block.base_curve, master_slabs)
                    
                    serialized_ext = []
                    for c in ext_crvs:
                        serialized_ext.extend(serialize_exact_curve(c))
                    ...
                    facade_dict[b_name][block.program].append({
                        ...
                        "BoundsClosed": serialize_exact_curve(block.base_curve),
                        "BoundsExt": serialized_ext
                    })

    exec_time = (time.perf_counter() - exec_start) * 1000 
    msg_lines = ["MP ENGINE V2", f"Time: {exec_time:.1f} ms", "---", f"Gross Area: {dash_total_area:,.1f} SQM"]
    for prog_name, prog_area in dash_programs.items(): msg_lines.append(f"  • {prog_name}: {prog_area:,.1f} SQM")
