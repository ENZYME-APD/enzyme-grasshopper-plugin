...
def extract_domain(desc):
    # Matches "0.0 to 1.0", "0 to 10", "0.5 - 2.5", etc.
    match = re.search(r'([-0-9\.]+)\s*(?:to|-|~)\s*([-0-9\.]+)', desc)
    if match:
        try:
    ...
    for line in inputs_block.split(';'):
        ...
        if "AddNumberParameter" in line:
            if "GH_ParamAccess.item" in line:
                ...
                min_val, max_val = extract_domain(desc)
                inputs.append(("slider_num", name, val, min_val, max_val))
                continue
        ...
        if "AddIntegerParameter" in line:
            if "GH_ParamAccess.item" in line:
                ...
                min_val, max_val = extract_domain(desc)
                inputs.append(("slider_int", name, val, min_val, max_val))
                continue
...
def process_file(filepath):
    ...
    for i, inp in enumerate(inputs):
        ...
        if t == "slider_num" or t == "slider_int":
            val = inp[2]
            min_v = inp[3] if inp[3] is not None else 0.0
            max_v = inp[4] if inp[4] is not None else get_slider_max(val)
            # Make sure max_v is >= min_v
            if max_v < min_v: max_v = min_v + 10.0
            auto_wire_lines.append(f'                Enzyme.Utils.AutoWireHelper.WireSlider(this, document, {i}, {min_v}, {max_v}, {val}, 330, {y_offset});')
        elif t == "toggle":
...
print(f"Processed {processed} files for auto-wiring improvements v7.")
