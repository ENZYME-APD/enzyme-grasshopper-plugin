...
namespace Enzyme.Components
{
    public class WindEngine : GH_Component
    {
        ...
        // "Comfort" here is a quick single-direction, single-threshold read, not a
        // validated Lawson/NEN8100-style wind comfort criterion.
        private const double COMFORT_SPEED_THRESHOLD_MS = 5.0;

        // --- Cache of the last raycast-heavy grid solve, so pure re-coloring or
        // re-height changes (CustomColors / HeatmapHeight) don't re-run the expensive
        // terrain/building raycasting. ---
        private string _cachedFingerprint;
        private Point3d[] _cachedGridPoints;
        private Vector3d[] _cachedDirs;
        private double[] _cachedSpeeds;
        private bool[] _cachedSolid;
        private bool[] _cachedValid;
        private int _cachedUCount;
        private BoundingBox _cachedBBox;
        private double _cachedSpacing;
        private double _cachedMinSpeed;
        private double _cachedMaxSpeed;
        private int _cachedActiveSensorCount;
        private int _cachedComfortableCount;
        ...
        private static double PlanarDistanceToBBox(Point3d pt, BoundingBox bbox)
        {
            double dx = Math.Max(Math.Max(bbox.Min.X - pt.X, pt.X - bbox.Max.X), 0.0);
            double dy = Math.Max(Math.Max(bbox.Min.Y - pt.Y, pt.Y - bbox.Max.Y), 0.0);
            return Math.Sqrt(dx * dx + dy * dy);
        }

        // Cheap O(n) shape checksum (sum + sum-of-squares of vertex coordinates).
        // Counts/bounding-box alone aren't enough to detect a reshape (e.g. dragging
        // terrain grips) that keeps the same vertex count and overall extents.
        private static string MeshShapeChecksum(Mesh m)
        {
            ...
            return verts.Count + ":" + m.Faces.Count + ":" + sum.ToString("R") + ":" + sumSq.ToString("R");
        }

        private static string BuildFingerprint(Mesh terrain, List<Mesh> buildings, Vector3d windDir, double speed, double height, double spacing,
            double wakeRatio, double slopeCoeff, double cornerTangency, double cornerBoost)
        {
            var sb = new StringBuilder();
            sb.Append(MeshShapeChecksum(terrain)).Append('|');
            sb.Append(buildings.Count).Append('|');
            foreach (var b in buildings)
            {
                sb.Append(MeshShapeChecksum(b)).Append('|');
            }
            sb.Append(windDir.ToString()).Append('|').Append(speed).Append('|').Append(height).Append('|').Append(spacing)
              .Append('|').Append(wakeRatio).Append('|').Append(slopeCoeff).Append('|').Append(cornerTangency).Append('|').Append(cornerBoost);
            return sb.ToString();
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (execute && terrain != null && baseWindDir.IsValid && speed > 0 && spacing > 0)
            {
                ...
                string fingerprint = BuildFingerprint(terrain, buildings, baseWindDir, speed, height, spacing, wakeRatio, slopeCoeff, cornerTangency, cornerBoost);
                if (fingerprint == _cachedFingerprint && _cachedGridPoints != null)
                {
                    usedCache = true;
                }
                ...
                int uCount;
                BoundingBox bbox;
                double minObservedSpeed, maxObservedSpeed;
                int activeSensorCount, comfortablePointCount;

                if (usedCache)
                {
                    gridPoints = _cachedGridPoints;
                    finalDirs = _cachedDirs;
                    finalSpeeds = _cachedSpeeds;
                    solidMasks = _cachedSolid;
                    validMasks = _cachedValid;
                    uCount = _cachedUCount;
                    bbox = _cachedBBox;
                    spacing = _cachedSpacing;
                    minObservedSpeed = _cachedMinSpeed;
                    maxObservedSpeed = _cachedMaxSpeed;
                    activeSensorCount = _cachedActiveSensorCount;
                    comfortablePointCount = _cachedComfortableCount;
                }
                else
                {
                    ...
                    uCount = (int)Math.Floor((bbox.Max.X - bbox.Min.X) / spacing) + 1;
                    int vCount = (int)Math.Floor((bbox.Max.Y - bbox.Min.Y) / spacing) + 1;
                    int cellCount = uCount * vCount;

                    gridPoints = new Point3d[cellCount];
                    finalDirs = new Vector3d[cellCount];
                    finalSpeeds = new double[cellCount];
                    solidMasks = new bool[cellCount];
                    validMasks = new bool[cellCount];
                    ...
                    // --- Precompute per-building bounding boxes / heights / max reach, for cheap AABB pruning ---
                    int buildingCount = buildings.Count;
                    BoundingBox[] buildingBBoxes = new BoundingBox[buildingCount];
                    double[] buildingHeights = new double[buildingCount];
                    double[] buildingMaxReach = new double[buildingCount];
                    for (int b = 0; b < buildingCount; b++)
                    {
                        ...
                        buildingBBoxes[b] = bb;
                        double h = Math.Max(bb.Max.Z - bb.Min.Z, 0.01);
                        buildingHeights[b] = h;
                        double wakeReach = Math.Min(h * wakeRatio, maxWakeRange);
                        buildingMaxReach[b] = Math.Max(wakeReach, spacing * CORNER_SEARCH_RADIUS_SPACING_MULTIPLIER);
                    }
                    ...
                    Parallel.For(0, gridPoints.Length, i =>
                    {
                        ...
                        for (int b = 0; b < buildingCount; b++)
                        {
                            ...
                            if (PlanarDistanceToBBox(pt, buildingBBoxes[b]) > 0.01) continue; // must be within bbox footprint to be inside the mesh
                        ...
                        else
                        {
                            ...
                            for (int b = 0; b < buildingCount; b++)
                            {
                                Mesh building = buildings[b];
                                if (building == null) continue;
                                if (PlanarDistanceToBBox(pt, buildingBBoxes[b]) > buildingMaxReach[b]) continue;
                    ...
                    minObservedSpeed = double.MaxValue;
                    maxObservedSpeed = double.MinValue;
                    activeSensorCount = 0;
                    comfortablePointCount = 0;
                    ...
                    _cachedGridPoints = gridPoints;
                    _cachedDirs = finalDirs;
                    _cachedSpeeds = finalSpeeds;
                    _cachedSolid = solidMasks;
                    _cachedValid = validMasks;
                    _cachedUCount = uCount;
                    _cachedBBox = bbox;
                    _cachedSpacing = spacing;
                    _cachedMinSpeed = minObservedSpeed;
                    _cachedMaxSpeed = maxObservedSpeed;
                    _cachedActiveSensorCount = activeSensorCount;
                    _cachedComfortableCount = comfortablePointCount;
                }
                ...
                Func<Point3d, int> findNearestGridIndex = (Point3d p) =>
                {
                    ...
                    // exact cell plus 1-ring neighborhood, in case the particle drifted
                    // between lattice cells; still O(1) instead of O(N).
            ...
            if (execute)
            {
                double reportedMin = _cachedMinSpeed == double.MaxValue ? 0.0 : _cachedMinSpeed;
                double reportedMax = _cachedMaxSpeed == double.MinValue ? 0.0 : _cachedMaxSpeed;
                double finalComfortPercent = _cachedActiveSensorCount > 0 ? ((double)_cachedComfortableCount / _cachedActiveSensorCount) * 100.0 : 0.0;
                string cacheNote = usedCache ? " (cached grid)" : "";
                Message = $"{this.NickName}\nTime: {sw.ElapsedMilliseconds} ms{cacheNote}\n---\n● Min Speed: {reportedMin:F1} m/s\n○ Max Speed: {reportedMax:F1} m/s\n● Comfort: {finalComfortPercent:F1}% (≤ {COMFORT_SPEED_THRESHOLD_MS:F1} m/s, single direction)";
            }
            else
            {
                Message = $"{this.NickName}\nSTATUS: SLEEPING";
