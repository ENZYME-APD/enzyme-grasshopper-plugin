...
# We'll replace the block from "GeometryBase baseGeom = null;" down to the last return before Transform
pattern = r'GeometryBase baseGeom = null;.*?if \(boundary == null\)\s*\{\s*AddRuntimeMessage\(GH_RuntimeMessageLevel\.Error, "Could not extract a valid boundary\."\);\s*return;\s*\}'
